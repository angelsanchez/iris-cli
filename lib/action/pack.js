...
function pack (destination) {
	...
	if ( destination.indexOf(cwd) === 0 ) {
		console.error('Destination folder[' + destination + '] cannot be inside of the folder to pack[' + cwd + ']. Use an outside destination like ../dist');
		process.exit(1);
	}
	...
	fs.unlinkSync( path.join(destination, IRIS_PATH_JS) );
	fs.unlinkSync( path.join(destination, IRIS_INIT_JS) );
...
function minifyHtml (files, dest, done) {
	...
	for ( i = 0; i < files.length; i++ ) {
		...
		content.push('iris.tmpl(\'' + files[i] + '\',\'' + data.replace(/\\/g, '\\\\').replace(/\'/g, '\\\'') + '\');');
	}
